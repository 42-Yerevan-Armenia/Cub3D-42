...
/*   Updated: 2023/01/18 18:26:15 by arakhurs         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_textures(t_all *all)
{
	...
	all->img.n_wall = mlx_xpm_file_to_image(all->mlx, WALL, &w, &h);
	if ((all->img.n_wall) == NULL)
		ft_error("❌ Can't Open N_Wall 🚧 Texture");
	all->img.s_wall = mlx_xpm_file_to_image(all->mlx, WALL, &w, &h);
	if ((all->img.s_wall) == NULL)
		ft_error("❌ Can't Open S_Wall 🚧 Texture");
	all->img.e_wall = mlx_xpm_file_to_image(all->mlx, WALL, &w, &h);
	if ((all->img.e_wall) == NULL)
		ft_error("❌ Can't Open E_Wall 🚧 Texture");
	all->img.w_wall = mlx_xpm_file_to_image(all->mlx, WALL, &w, &h);
